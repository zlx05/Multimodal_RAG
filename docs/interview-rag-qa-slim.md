# 多模态个人学习知识库 RAG 系统 — 面试速记版（简历对题，2026-08）

> **本版说明：** 由完整版 `interview-rag-qa.md` 瘦身而来，只保留「简历写了什么就准备什么」的题目；不涉及简历的题目已删除（末尾附跳过清单，被问到回查完整版）。题号沿用完整版，方便对照。
>
...
## 简历修改说明（2026-08）——面试官若问「简历数字怎么改了」这样答

### 改了什么（简历 bullet 2 对照）

| 简历原文（改前） | 改为（推荐） | 为什么 |
|---|---|---|
| 「BM25+向量→RRF 融合→**路由门控** + relevance 打分重排」 | 「BM25+向量→RRF 融合→**relevance 打分重排 + 每文档多样性上限（单 Collection + document_id 分区，对齐行业主流）**」 | 文档级路由门控 2026-08 已删除——74 题消融证明只救 1/74 题（见 Q5） |
| 「Recall@1 **0.465→0.977，MRR 0.983**」 | 「74 题评估集四路对比：纯向量 0.797 / 纯 BM25 0.811 / RRF 0.851 / 生产链路 0.838；目标文档集 R@1 **0.932**、R@5 1.000」 | 0.465→0.977 / MRR 0.983 是**迁移前 43 题**旧评估集口径；当前是 74 题口径（见 Q33） |

**改后完整 bullet 2（可直接替换）**：
> 混合检索 + 查询改写 + 量化评估：BM25 与向量双路召回→RRF 融合→relevance 打分重排 + 每文档多样性上限（单 Collection + document_id 分区，对齐行业主流，基于 74 题消融删除无效的文档级路由门控）；查询改写主路 + 原问题副路并行融合；自建 74 题评估集四路对比（纯向量 0.797 / 纯 BM25 0.811 / RRF 0.851 / 生产链路 0.838，目标文档集 R@1 0.932、R@5 1.000）；RAGAS 式 LLM-as-judge 零标注评估答案忠诚度 0.865。

### 修改理由（是口径变化，不是数字造假）

- **0.465→0.977 / MRR 0.983 是真实发生过的历史实测**：每文档一 Collection 的跨库 RRF 有「榜首平局」（各库 rank-1 分箱不可比），被 relevance 重排修复。这两个数后来被**两个动作取代**：① 评估集扩容 43→74 题（43 题接近天花板、区分度不足，看不出好坏）；② 单库迁移（`rag_all` + `document_id` 分区）让跨库 rank 制平局**从根上消失**，路由门控经 74 题消融证明只救 1/74 题后删除。
- 所以现在「全库 R@1 0.838 / 目标文档集 0.932」是新口径的真实水平。**这不是系统变差**：扩题是评估变严，单库是把 per-doc rank masking 去掉后的更真实全局竞争（详见 Q5、Q32）。
- 简历 bullet 1、bullet 3 无需改动——「多模态解析」「ReAct / 意图路由 / 补检」「Milvus/MySQL/Redis」「JWT / AI 审核 / 画像 / 会话」在当前代码里全部成立。

### 如果面试官直接问「简历上两个数字怎么对不上」

> 「这两个数不是一个口径。0.465→0.977 是我早期 43 题评估集、每文档一 Collection 架构下的修复记录——当时修的是跨库 RRF 榜首平局。后来我做了两件事：评估集扩容到 74 题（43 题区分度不足），架构迁到单 Collection + document_id 分区（行业主流），路由门控经消融证明只救 1/74 题、就删了。所以简历换成了当前 74 题口径：全库 R@1 0.838，限定目标文档集 R@1 0.932、R@5 1.000。指标口径跟着评估和架构走，这是评估体系在起作用，不是系统退化。」
...
| 简历要点 | 本版对应题 | 面试官大概率追问 |
|---|---|---|
| 项目简介：学生复习场景 · 统一解析 PDF/Word/PPT/截图错题/手写笔记 · 可追溯答案 | Q1/Q3 | 为什么做 RAG 不做微调（Q2 通用题已删）；多模态为什么难；可追溯怎么保证 |
| 多模态解析 + Agentic 检索：保留页码/OCR 置信度/表格结构 · 视觉模型入库前补充理解 · LangChain ReAct · 意图路由决定检索范围 · 证据不足多轮补检 | Q12/Q14、Q18-Q21、Q22-Q25、Q27、Q29/Q30 | 扫描 PDF 怎么解；图里逻辑关系；意图路由实现；死循环防法；证据够不够怎么判断 |
| 混合检索 + 查询改写 + 量化评估：BM25+向量→RRF→relevance 重排+多样性上限 · 改写+双路召回 · 74 题评估集 · 忠诚度 0.865 | Q4/Q5、Q8-Q11、Q31-Q34 | RRF 为什么不加权；为什么不用模型 reranker；评估集怎么建；数字可复现吗 |
| 完整工程化 + 个性化学习库：Milvus/MySQL/Redis 分工 · 增量入库不重建整库 · 用户隔离 · JWT 鉴权 · 上传 AI 审核 · 用户画像 · 会话持久化 | Q35/Q36、Q38-Q41 | 增量怎么维护；上传审核怎么做；鉴权为什么升级 JWT；画像怎么进路由 |
| 简历数字 / 路由门控改版 | 「简历修改说明」一节 | 为什么简历数字和面经对不上 |
...
## 本版删除的题目（不涉及简历，先不背；被问到回查完整版）

| 删除的题 | 内容 | 什么时候需要补 |
|---|---|---|
| Q2 | RAG vs 微调/长上下文 | 极高频通用题，几乎每个 RAG 项目都会被问，**建议补** |
| Q6 | Embedding 选型 | 被追问「为什么 bge-small」时（简历没写，追问率中） |
| Q7 | BM25 中文分词 | 被追问「中文怎么分」时（简历没写，追问率中） |
| Q13 | chunk 大小 40/1600/500/60 | Q12 的深挖 |
| Q15/Q16 | 跨页合并 / HTML 回退 | 多模态深挖 |
| Q17 | Contextual Retrieval | 追问加分 |
| Q26 | 结构化输出 bind_tools + 花括号坑 | ReAct 实现深挖 |
| Q28 | Agentic 成本 4~8 倍 | 追问率中高，可一句话带过（限流 + token 预算 + 无收益开关全关） |
| Q37 | 可靠性/降级/状态机 | 工程深挖 |
| Q42 | 成本控制 | 追问率中 |
| Q43/Q44/Q45 | 上限 / 翻车经历 / 生产缺口 | 开放性压轴，**建议至少备 Q44 一个翻车故事**（花括号注入、门控阈值失效、HTML 回退三选一） |
