...
<template>
  <section class="chunks-layout fade-up">
    <div class="chunks-main">
      <div class="chunk-toolbar">
        ...
        <div class="field-block compact-field">
          <label class="field-label" for="chunk-document">查看资料</label>
          <select id="chunk-document" v-model="selectedDocument" class="control-select">
            <option value="">选择已入库资料</option>
            <option v-for="document in store.documents" :key="document.document_id" :value="document.document_id">{{ document.topic_label || document.filename }} · {{ document.filename }}{{ document.task_status === "FAILED" ? "（处理失败）" : "" }}</option>
          </select>
        </div>
      ...
      <div v-if="isFailedDoc" class="empty-state large"><WarningCircle :size="28" /><strong>这份资料处理失败了</strong><span>它没有可查看的切片。可删除残留后重新上传，或直接重试原任务。</span></div>
      <div v-else-if="loading" class="skeleton-panel"><div class="skeleton-line wide"></div><div class="skeleton-line"></div><div class="skeleton-line short"></div></div>
      <div v-else-if="errorMessage" class="empty-state large"><WarningCircle :size="28" /><strong>暂时无法展示切片</strong><span>{{ errorMessage }}</span></div>
      <div v-else-if="!chunks.length" class="empty-state large"><div class="empty-graphic"><FileMagnifyingGlass :size="27" /></div><strong>选择一份资料开始检查</strong><span>切片会按原始顺序排列，点击任意区块查看完整内容和来源元数据。</span></div>
      <template v-else>
